...
@section('breadcrumb')
    @component('components.Breadcrumb')
        ...
        <li><a class="breadcrumb_currentPage" href="{{ route('users.edit', $user->id) }}">ویرایش کاربر ({{ $user->full_name }})</a></li>
    @endcomponent
@endsection
...
@section('content')
    ...
    <div class="row">
        <div class="col hi-subjectDashboardAdminProfile text-right">
            <h5 class="hi-subjectDashboardAdminProfile_h5">تاریخ ثبت نام {{ $user->create_date() }}<span> &nbsp;|&nbsp; </span>تاریخ آخرین ویرایش
                اطلاعات : {{ $user->update_date() }}</h5>
        </div>
    </div>
    {!! Form::model($user , ['method'=>'PUT', 'action'=>['AdminControllers\UserController@update', $user->id], 'files' => true, 'id'=>'form']) !!}
    <div class="row">
                    ...
                    <div class="row">
                        ...
                        <div class="col-3">
                            <div class="form-group">
                                ...
                                {!! Form::text('postal_code', null, ['class' => 'form-control hi-aboutMePanelCard_input', 'placeholder' => 'کد پستی ده رقمی', 'tabindex' => '8']) !!}
                            </div>
                        </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-4 px-2 hi-profileCard_PictureSelectorBox_pictureBox">
                            <div class="hi-profileCard_PictureSelectorBox_pictureBox_hover">
                                @if(isset($user->photos[0]->name))
                                    {!! Form::text('indexPhoto', $user->photos[0]->id, ['style' => 'display: none;']) !!}
                                @else
                                    {!! Form::text('indexPhoto', null, ['style' => 'display: none;']) !!}
                                @endif
                                <figure data-toggle="modal" data-target="#galleryModal">
                                    @if(isset($user->photos[0]->name))
                                        <img src="{{ asset('gallery' . '/' . $user->photos[0]->name) }}"
                        ...
                        <div class="col-4 pt-5">
                            <div class="hi-profileCard_PictureSelectorBox_selector hi-profileCard_PictureSelectorBox_selector_first mt-4">
                                {!! Form::select('role_id',$roles, $user->roles()->first()->id, ['class'=>'dropdown', 'data-settings'=>'{"wrapperClass":"metro"}']) !!}
                            </div>
                        </div>
                    </div>
                </div>
